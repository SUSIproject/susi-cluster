...
Susi::ClusterComponent::ClusterComponent(Susi::SusiClient &susi,
                                         BSON::Value &config)
  ...
  if (_config["nodes"].isArray()) {
    ...
    for (std::size_t nodeId = 0; nodeId < _config["nodes"].size(); nodeId++) {
      ...
      registerNode(node["id"], node["addr"],
                   static_cast<short>(node["port"].getInt64()), node["key"],
                   node["cert"]);
      if (node["forwardProcessors"].isArray()) {
        for (std::size_t i = 0; i < node["forwardProcessors"].size(); i++) {
          ...
          std::cout << "setup processor forwarding of " << topic << " to "
                    << node["id"].getString() << std::endl;
        }
      }
      if (node["forwardConsumers"].isArray()) {
        for (std::size_t i = 0; i < node["forwardConsumers"].size(); i++) {
          ...
          std::cout << "setup consumer forwarding of " << topic << " to "
                    << node["id"].getString() << std::endl;
        }
      }
      if (node["registerConsumers"].isArray()) {
        for (std::size_t i = 0; i < node["registerConsumers"].size(); i++) {
          ...
          std::cout << "register consumer for " << topic << " at "
                    << node["id"].getString() << std::endl;
        }
      }
      if (node["registerProcessors"].isArray()) {
        for (std::size_t i = 0; i < node["registerProcessors"].size(); i++) {
          ...
          std::cout << "register processor for " << topic << " at "
                    << node["id"].getString() << std::endl;
        }
      }
    }
  } else {
    std::cout << "config[nodes] is not an array (malformed config file?)"
              << std::endl;
  }
}

void Susi::ClusterComponent::join() { _susi.join(); }

void Susi::ClusterComponent::registerNode(std::string id, std::string addr,
                                          short port, std::string key,
                                          std::string cert) {
  _nodes[id] = std::make_shared<Susi::SusiClient>(addr, port, key, cert);
}

bool Susi::ClusterComponent::forwardProcessorEvent(std::string topic,
                                                   std::string id) {
  ...
  _susi.registerProcessor(topic, [node, this, id](Susi::EventPtr event) {
    std::cout << "forward processor " << event->topic << " to " << id
              << std::endl;
    if (!node->connected()) {
    ...
    struct FinishCallback {
      ...
      std::string id;
      FinishCallback(Susi::EventPtr evt) : localEvent{std::move(evt)} {}
...
bool Susi::ClusterComponent::forwardConsumerEvent(std::string topic,
                                                  std::string id) {
  if (_nodes.count(id) == 0) {
    return false;
  }
...
bool Susi::ClusterComponent::registerProcessor(std::string topic,
                                               std::string id) {
  if (_nodes.count(id) == 0) {
    return false;
  }
  ...
  node->registerProcessor(topic, [this, topic, id](Susi::EventPtr remoteEvent) {
    ...
    struct FinishCallback {
      ...
      FinishCallback(Susi::EventPtr evt) : remoteEvent{std::move(evt)} {}
      FinishCallback(FinishCallback &&other)
          : remoteEvent{std::move(other.remoteEvent)} {}
      FinishCallback(FinishCallback &other)
          : remoteEvent{std::move(other.remoteEvent)} {}
      void operator()(Susi::SharedEventPtr localEvent) {
        std::cout << "send ack for event " << localEvent->topic
                  << " back to origin" << std::endl;
...
bool Susi::ClusterComponent::registerConsumer(std::string topic,
                                              std::string id) {
  if (_nodes.count(id) == 0) {
    return false;
  }
  auto &node = _nodes[id];
  node->registerConsumer(topic, [this, topic, id](
                                    Susi::SharedEventPtr remoteEvent) {
